...
private:
    ...
    unsigned int _size;  	//size of binary tree
...
protected:
  ...
  **/
  void _recursivelyDelete(Iterator iter) {
    ...
    //want postorder traversal to delete nodes:
    //base case: do nothing if iter is a NULL 
    if (!iter.isNULL()) {
  ...
  * Input parameter: iterator pointing to subtree that should be copied
  * Return a pointer to the root of the duplicate tree
  **/
  Node* _duplicate(Iterator iter) {
    //pre-order traversal to recursively copy
    if (!iter.isNULL()) {
      Node* temp = new Node(*iter);
    
      Node* leftchild, rightchild;
      if (iter.hasLeftChild()) {
        leftchild = _duplicate(iter.left());
        leftchild->_parent = temp;
        temp->_left = leftchild;
      }
      if (iter.hasRightChild()) {
        rightchild =_duplicate(iter.right());
        rightchild->_parent = temp;
        temp->_right = rightchild;
      }
      return temp;
    }
    else
...
public:
  ...
  */
  ...
  void createRoot(const ItemType& value) {
    if (!empty())
      throw runtime_error("we already have a root");
