...
//HOMEWORK
  ...
  T& operator[](int index) {
   //error checking if list is empty or index is not valid
   if(empty()){
      throw domain_error("list is empty");}
   if(index>_size|| index<0){
      throw domain_error("index is not valid");} 
  ...
  void replace(T val1, T val2) {
   if(empty()){
     throw domain_error("list is empty");}
   if(val1==val2){
     throw runtime_error("values are the same, no need to replace");}
   iterator it;
  ...
  void reverse() {
   if(empty()){
    throw domain_error("list is empty");}
   if(_size==1){
    throw domain_error("list is length one, no need to reverse");}
   //pointers to hold values
   Node* temp;
   Node* start = _sent;
   //start the base case at the sent node
   temp = start -> _next;
   start -> _next = start -> _previous;
   start -> _previous = temp;
   start = start -> _previous;
   //iterate through the rest of the list until reach the sent node
   while(start != _sent){
     temp = start -> _next;
     start -> _next = start -> _previous;
     start -> _previous = temp;
  ...
  void sort() {
   ...
   if(empty()){
    throw domain_error("list is empty");}
   if(_size==1){
    throw domain_error("list of size 1 is sorted");}
   for(int i=0; i!=size(); i++){
   //pointers to hold nodes
   Node* first= _sent -> _next;
   Node* second= _sent ->_next -> _next;
    while(second!= _sent){
     ...
     first = first -> _next;
     second = second -> _next;
    }//process will repeat for the number of elements to ensure it is sorted
   }
  }
